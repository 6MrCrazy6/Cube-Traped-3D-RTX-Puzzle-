	...
	void Teleport()
	{
		...
		if(instantTeleport)
		{
			...
			if(randomTeleport)
			{
				...
				subject.transform.position = destinationPad[chosenPad].transform.position + new Vector3(0, 0.5f, 0);
				//play teleport sound
				teleportSound.Play();
			}
			else //if not random, teleport to the first one in the array list
			{
				...
				//and teleport the subject
				subject.transform.position = destinationPad[0].transform.position + new Vector3(0, 0.5f, 0);
				//play teleport sound
				teleportSound.Play();
			}
		}
